...
## Запуск приложения для разработки в Docker
Данная инструкция предназначена для запуска приложения в докере.  
Перед началом вам необходимо выполнить настройку телеграм и файла `.env`. 
>*ВАЖНО*  
>у вас должен быть установлен Docker Desktop (Mac, Win) или Docker Engine (Linux).  
1. собираем образ: для этого, находясь в корне приложения введите одни из следующих команд
```
...
make docker-build
```
Будет собрано 4 образа:
- `tg-marketing-service-dev-adminer` - контейнер для управления БД (доступен на порту 8080)
- `tg-marketing-service-dev-db` - контейнер с БД
- `tg-marketing-service-dev-frontend` - контейнер для фронтэнда (доступен на порту 3000)
- `tg-marketing-service-dev-backend` - контейнер для бэкэнда (доступен на порту 8000)

2. создаем и запускаем контейнеры на основе созданных образов
```
docker compose up -d
```
или
...
- `tg-marketing-service-dev-adminer` - контейнер для управления БД [http://127.0.0.1:8080/](http://127.0.0.1:8080/)
- `tg-marketing-service-dev-db` - доступ через adminer
- `tg-marketing-service-dev-frontend` - контейнер для фронтэнда [http://127.0.0.1:3000/](http://127.0.0.1:3000/)
- `tg-marketing-service-dev-backend` - контейнер для бэкэнда [http://127.0.0.1:8000/](http://127.0.0.1:8000/)

3. Чтобы остановить приложение и удалить их введите:
```
docker compose down
```
или
